...
task coverage-only {
	exec { & .\src\packages\OpenCover.4.5.3427\OpenCover.Console.exe -register:user -target:vstest.console.exe "-targetargs:$testOptions ""src\csmacnz.Coveralls.Tests\bin\$Configuration\csmacnz.Coveralls.Tests.dll""" -filter:"+[csmacnz.Coveralls*]*" -output:opencovertests.xml }
}
