...
public class CoverageFileBuilderTests
{
    ...
    [Fact]
    public void NewBuilderWithNullFileThrowsArgumentException() => Assert.Throws<ArgumentNullException>(() => new CoverageFileBuilder(null!));

    public static CoverageFileBuilder CreateFileBuilder(string filePath = @"C:\temp\file.cs") => new CoverageFileBuilder(new FileCoverageData(filePath, new int?[1]));
    ...
    public class WhenCreatingACoverageFromADefaultBuilder
    {
        ...
        [Fact]
        public void ThenFileNameIsSet() => Assert.Equal(Filename, _coverageFile.Name);

        [Fact]
        public void ThenSourceDigestIsTheEmptyDigest() => Assert.Equal("D41D8CD98F00B204E9800998ECF8427E", _coverageFile.SourceDigest);
