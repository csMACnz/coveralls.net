...
public class NCoverTests
{
    ...
    public void EmptyReport_RunsSuccessfully()
    {
        ...
        var filePath = TestFileSystem.GenerateRandomAbsolutePath("ncover", "EmptyReport.xml");
    ...
    {
        var fileSystem = new TestFileSystem();
        var filePath = TestFileSystem.GenerateRandomAbsolutePath("ncover");

        var sourcePath = Path.Combine(filePath, "SingleFileReportSourceFile.txt");
        ...
        var reportPath = Path.Combine(filePath, "SingleFileReportOneLineCovered.xml");
        fileSystem.AddFile(reportPath, reportContents);

        return (fileSystem, reportPath);
    }

    private static CoverallsRunResults DryRunCoverallsWithInputFile(string inputFilePath, TestFileSystem testFileSystem) => CoverallsTestRunner.RunCoveralls(
            $"--ncover -i {inputFilePath} --dryrun --repoToken MYTESTREPOTOKEN",
            testFileSystem);

    private static CoverallsRunResults DryRunCoverallsMultiModeWithInputFile(string inputFilePath, TestFileSystem testFileSystem) => CoverallsTestRunner.RunCoveralls(
