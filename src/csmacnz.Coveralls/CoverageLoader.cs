...
public class CoverageLoader
{
    ...
    public CoverageLoader(IFileLoader fileLoader) => _fileLoader = fileLoader;
        ...
        bool useRelativePaths)
    {
        ...
        {
            var coverageData = loadResult.Value;
    ...
    private Option<List<FileCoverageData>> LoadData(string directory, [JetBrains.Annotations.NotNull] Func<Dictionary<string, XDocument>, List<FileCoverageData>> generateFunc)
    {
    ...
    private Dictionary<string, XDocument> LoadXDocuments(Option<FileInfo[]> folderFiles) => ((FileInfo[])folderFiles).Where(f => f.Name.EndsWith(".xml", StringComparison.Ordinal))
