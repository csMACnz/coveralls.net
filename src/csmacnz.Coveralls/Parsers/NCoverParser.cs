        ...
        public static List<FileCoverageData> GenerateSourceFiles(XDocument document)
        {
            ...
            if (xElement != null)
            {
                foreach (var module in xElement.Elements("module"))
                {
                    ...
                    {
                        XAttribute attribute = method.Attribute(XName.Get("excluded")) !;
                        ...
                        {
                            attribute = method.Attribute(XName.Get("excluded")) !;
                            if (!string.IsNullOrEmpty(attribute?.Value) &&
                                bool.Parse(attribute.Value))
                            {
                                    continue;
                            ...
                            var sourceLine =
                                int.Parse(seqpnt.Attribute(XName.Get("line")) !.Value, CultureInfo.InvariantCulture);
                            var visitCount =
                                int.Parse(seqpnt.Attribute(XName.Get("visitcount")) !.Value, CultureInfo.InvariantCulture);
                            var file = seqpnt.Attribute(XName.Get("doc" + "ument")) !.Value; // avoid false positive error CC0021: Use 'nameof(document)'
