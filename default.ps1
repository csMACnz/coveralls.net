...
task archive-only {
    ...
    mkdir $archive_dir
    dotnet publish $app_project -f netcoreapp3.1 -c $configuration -o "$archive_dir\windows" -r win-x64
    dotnet publish $app_project -f netcoreapp3.1 -c $configuration -o "$archive_dir\linux" -r linux-x64
    dotnet publish $app_project -f netcoreapp3.1 -c $configuration -o "$archive_dir\osx" -r osx-x64
